...
export function ActaDetailsSheet({
  actaId,
  numeroActa,
}: ActaDetailsSheetProps) {
      ...
      <SheetContent className="overflow-y-auto overflow-x-hidden w-[400px] sm:w-[600px] p-6 sm:p-8">
        ...
        {isLoading ? (
          ...
          <div className="space-y-8 pb-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-6">
                ...
                {renderField(
                  'Servidor Saliente',
                  details.nombreServidorSaliente
                )}
                {renderField(
                  'Designación (Saliente)',
                  details.designacionServidorSaliente
                )}

                {/* Recibe */}
                {renderField(
                  'Servidor que Recibe',
                  details.nombreServidorRecibe
                )}
                {renderField(
                  'Designación (Recibe)',
                  details.designacionServidorRecibe
                )}

                {/* Entrante */}
                {renderField(
                  'Servidor Entrante',
                  details.nombreServidorEntrante
                )}
                {renderField(
                  'Designación (Entrante)',
                  details.designacionServidorEntrante
                )}
              </div>
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-40 text-muted-foreground bg-muted/30 rounded-lg border border-dashed">
            <p className="text-sm">
              No se encontró información disponible para esta acta.
            </p>
          </div>
        )}
      </SheetContent>
