...
export function ComplianceDetailsSheet({ actaId, numeroCompliance }: ComplianceDetailsSheetProps) {
  const [isOpen, setIsOpen] = React.useState(false);
  const [isLoading, setIsLoading] = React.useState(false);
  const [details, setDetails] = React.useState<ComplianceInfoDetails | null>(null);
    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      <SheetContent className="overflow-y-auto overflow-x-hidden w-[400px] sm:w-[600px] p-6 sm:p-8">
        <SheetHeader className="mb-8 space-y-2">
          <SheetTitle className="text-2xl font-bold text-primary">Detalles de Compliance</SheetTitle>
          <SheetDescription className="text-base">
            Información del evaluador para el acta <span className="font-medium text-foreground">{numeroCompliance || 'S/N'}</span>.
          </SheetDescription>
        </SheetHeader>
